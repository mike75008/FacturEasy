...
const s = StyleSheet.create({
  ...
  // Header : entreprise + type document
  header: {
    flexDirection: "row",
    ...
    >
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          <View>
            ...
            {organization.address && (
              <Text style={s.companyInfo}>{organization.address}</Text>
            )}
            ...
            {organization.phone && (
              <Text style={s.companyInfo}>Tél : {organization.phone}</Text>
            )}
            {organization.email && (
              <Text style={s.companyInfo}>{organization.email}</Text>
            )}
            {organization.siret && (
              <Text style={s.companyInfo}>SIRET : {organization.siret}</Text>
            )}
            {organization.tva_number && (
              <Text style={s.companyInfo}>TVA : {organization.tva_number}</Text>
            )}
          </View>
          ...
          <View>
            ...
            {doc.due_date && (
              <Text style={s.docMeta}>Échéance : {fDate(doc.due_date)}</Text>
            )}
          </View>
        </View>
        ...
        <View style={s.clientBlock}>
          ...
          {client?.address && (
            <Text style={s.clientInfo}>{client.address}</Text>
          )}
          ...
          {client?.email && (
            <Text style={s.clientInfo}>{client.email}</Text>
          )}
          {client?.siret && (
            <Text style={s.clientInfo}>SIRET : {client.siret}</Text>
          )}
          {client?.tva_number && (
            <Text style={s.clientInfo}>TVA : {client.tva_number}</Text>
          )}
        </View>
            ...
            {doc.discount_amount > 0 && (
              <View style={s.totalRow}>
                <Text style={s.totalLabel}>
                  Remise ({doc.discount_percent}%)
                </Text>
                <Text style={s.totalDiscount}>
                  -{fCurrency(doc.discount_amount)}
                </Text>
              </View>
            )}
            <View style={s.totalRow}>
        ...
        {(organization.rib_iban || organization.rib_bic) && (
          <View style={s.ribBlock}>
            ...
            {organization.rib_iban && (
              <Text style={s.ribText}>IBAN : {organization.rib_iban}</Text>
            )}
            {organization.rib_bic && (
              <Text style={s.ribText}>BIC : {organization.rib_bic}</Text>
            )}
            {organization.rib_bank && (
              <Text style={s.ribText}>Banque : {organization.rib_bank}</Text>
            )}
          </View>
        )}
